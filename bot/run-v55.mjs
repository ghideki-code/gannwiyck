...
await fs.writeFile(path.join(OUT,'SUMMARY.md'),'# GannWyck V5.5 Robustness\n\n'+all.map(r=>'## '+r.source.timeframe+'\n'+Object.entries(r.candidates).map(([k,v])=>{const z=v.robustness;return '- **'+k+'**: '+v.all.closed+' closed, '+v.all.totalR.toFixed(2)+'R, top3-removal '+(z.removeTop.find(x=>x.k===3)?.totalR??NaN).toFixed(2)+'R, block-bootstrap P05 '+(z.blockBootstrap?.p05==null?'NA':Number(z.blockBootstrap.p05).toFixed(2))+'R, second half '+z.secondHalf.totalR.toFixed(2)+'R'}).join('\n')).join('\n\n'))})();
